...
const ImportancePlot: React.FC<ImportancePlotProps> = (props) => {
  ...
  const xAbsoluteMaxValue = useMemo(() => {
    ...
    return Math.max(absValue * 1.1, 0.1);
  }, [data]);
  ...
  const getStableJitter = (value: number, index: number, bandWidth: number) => {
    ...
    const jitterRange = Math.min(bandWidth * 0.3, 15);
      ...
      <div ref={containerRef} style={{ width: "100%", marginBottom: "20px" }}>
        ...
        >
          <Group left={margin.left} top={margin.top}>
            {data.map((d) => {
              const barY2 =
                (yScale(d.name) ?? 0) + (yScale.bandwidth?.() ?? 0) / 2;
              return (
                <line
                  key={`line1-${d.name}`}
                  x1={0}
            ...
            {toggle ? (
              ...
              <>
                {data.map((d) => {
                  ...
                  const textX = d.value > 0 ? barX + 5 : barX + barWidth - 5;
                  const textAnchor = d.value > 0 ? "start" : "end";
