...
if __name__ == '__main__':
    ...
    while True:
        count +=1
        state, ret = opt.step(state)
        if len(state.trials):
            trial = state.trials.pop()
            loss = opt.evaluating(state, trial['config'], iter=trial['iter'])
            print("loss", loss)
        else:
            loss = opt.evaluating(state, ret[0].to_dict())
        state = opt.done(state, ret, loss)

        # # test trial
        # if count==50:
        #     state = opt.add_trial(state, trial)
        
        # test to narrow config space
        if count == 20:
            state = opt.narrow_configspace(name='batch_size', state=state, value=8)
        
        if count == 30:
            state = opt.narrow_configspace(name='lr', state=state, lower=1e-3, upper=1e-1)
